#!/usr/bin/env python3
"""Test: launch outcast, barrier, then launch gather separately."""
...
# Step 1: Launch outcast only
if rank == 0:
    print("Step 1: Launching outcast...", flush=True)
ha.outcast_only(output_embs, gather_idx, offsets,
                peer_slot_ptrs_dev, peer_sig_ptrs_dev, rank, L, D, 1)
torch.cuda.synchronize()
dist.barrier()
if rank == 0:
    print("Step 1: Outcast done, signals should be set.", flush=True)

# Step 2: Launch two_kernel (outcast will re-signal with iter_id=2, gather waits for 2)
if rank == 0:
    print("Step 2: Launching two_kernel...", flush=True)
...
torch.cuda.synchronize()
if rank == 0:
    print(f"Step 2: two_kernel done! shape={result.shape}", flush=True)
